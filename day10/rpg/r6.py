...
imgEffect = pygame.image.load(f"{images}effect_a.png")
imgEnemy = pygame.image.load(f"{images}enemy4.png")
...
def draw_battle(bg, fnt):
    ...
    # emy_blinkに値が入ったときは点滅処理emy_blinkが奇数の時はキャラを描画しない(背景のみ)
    if emy_blink % 2 == 0:
        # 敵を描画
        bg.blit(imgEnemy, [emy_x, emy_y+emy_step])
    if emy_blink > 0:
        # emy_blinkの値を毎フレーム１減らす
